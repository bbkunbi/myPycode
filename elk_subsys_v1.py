#------------------------------------------------------------------------------------------------------|
#   Date: 21-Jan-2022 || code is used to send data in ELK stack in particular index only for all subsys
#   Author: Bhavesh Kunbi, Telemetry Lab
#   elk_subsys_v1.py || 31.1.2022 bug fixed dict overwrite ||
#
#------------------------------import libraries--------------------------------------------------------|
...
for subsys_index in subsys:
    try:
        ...
        print("index is already exist in kiban")
#--------------------------------- Dictionary created ------------------------------------------------|
dict1 = {}
...
def attributes():
    ...
    #print("Total attributes in",item2," system is" ,length)
        ...
        if (dict1["port"] == 3001.0):
            ...
            res = es.index(index="servo", doc_type = "doc", body = dict1 )
        elif (dict1["port"] == 3002.0):
            #print(">>>>> created OFCSNT subsys index.....")
            print("sending data in ELK index is [ofcsnt] ........")
            res = es.index(index="ofcsnt", doc_type = "doc", body = dict1 )
        else:
